...
yrp = seq(2021 + 1, 2021 + nproy+1, 1) #años futuros
...
for(l in 1:nsim){
  ...
  for(j in 1:nhcr){
    ...
    B[l,1,j] <- m1$ref_ts$b[1]
    Ft[l,1,j] <- C[l,1,j]/B[l,1,j]
    B_Bmsy[l,1,j] <- m1$ref_ts$bbmsy[1]
    for(i in 2:nyr){
      C[l,i,j] = m1$ref_ts$catch[i]
      B[l,i,j] = m1$ref_ts$b[i]
...
p3 <- ggplot(data=df,aes(x=Year,y=Mediana,group=HCR))+
  ...
  #geom_ribbon(aes(ymin = L1,ymax= L2),fill="grey30")+
  geom_line()+
  geom_hline(yintercept = fmsy,col="green")+
  facet_wrap(~HCR,ncol=4)+
...
ggsave("Figuras/Fig_6_Proyeccion_Biomasa.png",plot=p1)
ggsave("Figuras/Fig_7_Proyeccion_Capturas.png",plot=p2)
ggsave("Figuras/Fig_8_Proyeccion_MortPesca.png",plot=p3)
ggsave("Figuras/Fig_9_Proyeccion_razon_B-Bmsy.png",plot=p4)
