...
setwd("~/01Cursos/EvalManPesqLim2024/Lab/AMSY")
...
# If the line below is commented out (#), all stocks in the input file will be analyzed
 stocks <-  "ChaDa" #"AncudBay"   #"HL_VL"   #c("anb-78ab")  # c("HH_VL","HL_VL","HLH_VL","LH_VL","LHL_VL","LL_VL") # "anb-78ab"  #c("HL_H","HL_M","HL_L","HL_VL")  #"HH_L"  #"LHL_L"  #"Micr_pou_AD" #"Myxine glutinosa"   #"Eut_gurn_Balt"  # "rjc.27.3a47d"  #"PNSK"   # "Ille_coi_AD" # "WSTM"
...
#-----------------------------------------
smooth.cpue  <- TRUE # set to TRUE to apply ksmooth with minimum bandwidth of 3, increased at low r
...
sigma.cpue   <- 0.2 # observation error for cpue 
...
creep.graph  <- FALSE # plot graph for effort creep correction, if used
do.plots     <- TRUE # retrospective analysis does not work if FALSE
write.output <- FALSE # set to TRUE if table with results in output file is wanted I CHANGED THIS TO F
kobe.plot    <- TRUE # HW set to TRUE so produce additional kobe status plot 
save.plots   <- FALSE # set to TRUE to save graphs to JPEG files #  TO AVOID ERRORS ON MAC, NOT TO SAVE PLOTS AND SAVE THEM LATER 
close.plots  <- FALSE # set to TRUE to close on-screen plots, to avoid "too many open devices" error in batch-processing; I CHANGED THIS TO F
retros       <- FALSE # retrospective analysis, requires do.plots <- TRUE
...
#--------------------------------------------
if(write.output==TRUE && substr(id_file,1,3)=="Sim"){ # output for simulated data
...
if(write.output==TRUE && is.null(cinfo$MSY.BSM)==FALSE) { # assuming all BSM fields are available
  outheaders = data.frame("Stock","r.BSM","lcl","ucl","r.est","lcl","ucl","k.BSM","lcl","ucl","k.est","lcl","ucl",
                          "BBmsy.BSM","lcl","ucl","BBmsy.est","lcl","ucl",
                          "FFmsy.BSM","lcl","ucl",
                          "FFmsy.est","lcl","ucl") 
  write.table(outheaders,file=outfile, append = TRUE, sep=",",row.names=FALSE,col.names=FALSE)
  }

if(write.output==TRUE && substr(id_file,1,3)!="Sim" && is.null(cinfo$MSY.BSM)==TRUE){
...
for(stock in stocks) {
  ...
  if(retros==TRUE && (cinfo$EndYear[cinfo$Stock==stock]-cinfo$Bk.yr[cinfo$Stock==stock])<3) {
...
if (retros.nyears>0 && do.plots==TRUE){
...
#---------------------------------------	
	if(write.output==TRUE && substr(id_file,1,3)=="Sim") {
	  ...
	  write.table(output, file=outfile, append = TRUE, sep = ",", 
	...
	if(write.output==TRUE && substr(id_file,1,3)!="Sim" && is.null(cinfo$MSY.BSM)==TRUE){ # regular assessment
